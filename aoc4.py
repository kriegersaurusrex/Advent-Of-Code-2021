...
#Sees if sum for a given row, column, or diagonal is > 5000 which indicates a winning pattern for a given board
def test_winning_card(card_test):
    for i in range(0, 4):
        if(sum(card_test[i]) > 5000):
            return True
    for col_num in range(0, 4):
        if(card_test[0][col_num]+card_test[1][col_num]+card_test[2][col_num]+card_test[3][col_num]+card_test[4][col_num] > 5000):
            return True 
    if(card_test[0][0]+card_test[1][1]+card_test[2][2]+card_test[3][3]+card_test[4][4] > 5000):
        return True
    if(card_test[0][4]+card_test[1][3]+card_test[2][2]+card_test[3][1]+card_test[4][0] > 5000):
        return True
    return False

bingo_list = [0,56,39,4,52,7,73,57,65,13,3,72,69,96,18,9,49,83,24,31,12,64,29,21,80,71,66,95,2,62,68,46,11,33,74,88,17,15,5,6,98,30,51,78,76,75,28,53,87,48,20,22,55,86,82,90,47,19,25,1,27,60,94,38,97,58,70,10,43,40,89,26,34,32,23,45,50,91,61,44,35,85,63,16,99,92,8,36,81,84,79,37,93,67,59,54,41,77,42,14]
final_card = []
bingo_ctr = 0
final_num = 0
found_winner = False
#Iteratively fills in matching numbers on all bingo cards, and drops the bingo card if it's determined to be a winner until the there is only one card remaining & calculates its score
while(len(cards) > 0):
    for i in range(0, len(cards)):
        for j in range(0, len(cards[i])):
            if bingo_list[bingo_ctr] in cards[i][j]:
                for k in range(0, len(cards[i][j])):
                    if(bingo_list[bingo_ctr] == cards[i][j][k]):
                        cards[i][j][k] += 1000
    for i in range(0, len(cards)):
        winning_card = test_winning_card(cards[i])
        if(winning_card == True):
            found_winner = True
            if(len(cards) == 1):
                final_card = cards[0]
                final_num = bingo_list[bingo_ctr]
            cards.pop(i)
            break
    if(found_winner == False):
        ...
        found_winner = False
new_sum = 0
for i in range(0, len(final_card)):
    for j in range(0, len(final_card[0])):
        if(final_card[i][j] < 100):
            new_sum += final_card[i][j]
print("Final losing card score:", new_sum * final_num)

# Pt 1- Looks through all rows in all bingo cards to see if a number has been called, and returns the score of the score of the winning bingo card
# bingo_list = [0,56,39,4,52,7,73,57,65,13,3,72,69,96,18,9,49,83,24,31,12,64,29,21,80,71,66,95,2,62,68,46,11,33,74,88,17,15,5,6,98,30,51,78,76,75,28,53,87,48,20,22,55,86,82,90,47,19,25,1,27,60,94,38,97,58,70,10,43,40,89,26,34,32,23,45,50,91,61,44,35,85,63,16,99,92,8,36,81,84,79,37,93,67,59,54,41,77,42,14]
# final_card = []
# bingo_ctr = 0
# final_num = 0
...
#         winning_card = test_winning_card(cards[a])
#         if(winning_card == True):
#             final_num = bingo_list[bingo_ctr]
#             final_card = cards[a]
#             found_winner = True
...
#         if(final_card[i][j] < 100):
#             new_sum += final_card[i][j]
# print("Final winning card score:", new_sum * final_num)